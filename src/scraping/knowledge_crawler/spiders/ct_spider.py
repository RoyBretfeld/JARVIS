...
import queue # Sicherstellen, dass queue importiert ist (kann nicht schaden)

# Logger spezifisch für den Spider
spider_logger = logging.getLogger("CtSpider")

class CtSpider(scrapy.Spider):
    name = "ct_spider"
    # c't ist Teil von heise.de
    allowed_domains = ["heise.de"]
    start_urls = ["https://www.heise.de/ct/"]

    # Hinweis: Consent-Management beachten (analog zu Golem/Heise)
    # Ggf. Cookie oder andere Logik notwendig.
    ...
    def __init__(self, learning_manager=None, result_queue=None, *args, **kwargs):
        ...
        # Verwende den Standard-Logger des Spiders
        self.logger.info(f"CtSpider initialisiert {'mit' if learning_manager else 'ohne'} LearningManager und {'mit' if result_queue else 'ohne'} Result Queue.")
    ...
    def _handle_spider_closed(self, spider, reason):
        ...
        self.logger.info(f"CtSpider ({self.name}) geschlossen (lokaler Handler). Grund: {reason}")

    def _handle_spider_error(self, failure, response, spider):
        if self.result_queue:
            try:
                error_msg = f"Spider Error in {spider.name}: {failure.getErrorMessage()}"
                # Logge den Fehler hier auch, bevor er in die Queue geht
    ...
    def parse(self, response):
        ...
        # === Angepasste Selektoren für c't Links (basierend auf test_ct_selectors.py) ===
        selectors = [
            'a[href*="/news/"]::attr(href)',      # Scheint die meisten relevanten Links zu finden
            'a[href*="/artikel/"]::attr(href)'   # Findet auch einige Links
            # Alte, nicht mehr funktionierende Selektoren entfernt.
        ]
        article_links = set()
        for selector in selectors:
            ...
            if found_links:
                ...
                for link in found_links:
                     # Prüfe, ob es ein valider relativer oder absoluter Link ist
        ...
        if not article_links:
             ...
             # Optional: Status-Update an Queue senden, dass keine Links gefunden wurden?
             # if self.result_queue:
             #     self.result_queue.put(('status', f'Keine Links auf {response.url} gefunden'))

        for article_url in article_links:
            if article_url and self.allowed_domains[0] in article_url:
                self.logger.debug(f"Folge Artikel-Link: {article_url}")
                # Optional: Status-Update an Queue senden?
                # if self.result_queue:
                #     self.result_queue.put(('status', f'Folge Link: {article_url[-50:]}'))
    ...
    def parse_article(self, response):
        ...
        # === Korrekte Selektoren für c't Artikelinhalte (basierend auf test_ct_article_parser.py) ===
        ...
        if title and article_text:
            ...
            # Optional: Status-Update für extrahierten Artikel?
            # if self.result_queue:
            #     self.result_queue.put(('status', f'Artikel extrahiert: {item["title"][:30]}...'))
        elif not title:
